...
/**
 * Erzeugt eine neues UploadManager-Objekt.
 *
...
var UploadManager = function(req, resp, log_client, upload_dir) {
    this._req = VC.instance_of(
        req, 'req', http.IncomingMessage
    );
    this._resp = VC.instance_of(
        resp, 'resp', http.ServerResponse
    );
    this._log_client = VC.instance_of(
        log_client, 'log_client', LogClient
    );
    this._upload_dir = VC.string(upload_dir, 'upload_dir');
};
