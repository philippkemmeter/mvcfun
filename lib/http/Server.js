 ...
 *        - allowedMethods:     Array of http methods allowed. All methods are
 *                              allowed by default. Controllers may disable
 *                              methods on ressource base.
 *        - reqBodyMaxLength:   The max length of the request body. You may
 *                              disable this security feature by setting this
 *                              to a negative value. Defaults to 1000.
 *        - logLevel:           The higher the level, the more is logged by
 *                              emitting the log event.
 *                              Has to be an int value between -1 and 5.
 *                              Defaults to 0, i.e. that the log event is
 *                              emitted manually in controllers, only.
 *        - requestManager      The request manager to use.
 *                              If omitted, the default manager will be
 *                              created.
 *        - responseManagers    The response managers to use.  You may register
 *                              one per content type
 *                              Defaults to:
 *                              {
 *                               'text/html':        response.manager.Html,
 *                               'text/plain':       response.manager.Plain,
 *                               'application/json': response.manager.Json
 *                              }
 *        - defaultContentType: The content type the server serves by default,
 *                              i.e. all error pages will be returned using
 *                              this content type. Controllers may use this
 *                              information to adapt their output respectively.
 *
 * @constructor
 */
module.exports = function(options) {
    ...
    if (options && options.defaultContentType)
        this.defaultContentType = options.defaultContentType;

    if (options && options.responseManagers) {
        for (var contentType in options.responseManagers) {
            this.responseManagers[contentType]
                = options.responseManagers[contentType];
        }
    }
    if (!this.responseManagers['text/html'])
        this.responseManagers['text/html'] = new response.manager.Html();
    if (!this.responseManagers['text/plain'])
        this.responseManagers['text/plain'] = new response.manager.Plain();
    if (!this.responseManagers['application/json'])
        this.responseManagers['application/json'] = new response.manager.Json();

    for (var contentType in this.responseManagers) {
        this.responseManagers[contentType]
            .on('log', function(msg) { _this._log(msg); })
    ...
    if (options && options.requestManager) {
        ...
        this.requestManager = new request.Manager(
            this.responseManagers,
            (function(_this) {
                return function() {
                    return _this.responseManager;
                }
            })(this),
...
/**
 * The default response manager, i.e. the response manager of the default
 * content type.
 *
...
nepp.createGS(module.exports.prototype, 'responseManager',
    function getResponseManager() {
        return this._responseManagers[this._defaultContentType];
    },
    function setResponseManager(responseManager) {
        this._responseManagers[this._defaultContentType] = VC.instance_of(
 ...
 */
nepp.createGS(module.exports.prototype, 'defaultContentType',
    function getDefaultContentType() {
        return this._defaultContentType;
    },
    function setDefaultContentType(contentType) {
        this._defaultContentType = VC.string(contentType, 'contentType');
    }
);
